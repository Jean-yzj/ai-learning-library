...
window.SITE_DATA = {
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          priceNote: "有免費方案，用 Google 帳號即可開始。",
  ...
  glossary: [
    ...
    { term: "機器學習", en: "Machine Learning", def: "讓電腦從大量資料中自己「學會」規律，而不是人一條條寫死規則。是 AI 的基礎方法。", see: ["深度學習", "模型"] },
    { term: "深度學習", en: "Deep Learning", def: "機器學習的一支，用多層神經網路處理複雜資料（影像、語音、文字），是現代 AI 爆發的核心。", see: ["機器學習", "神經網路"] },
    { term: "神經網路", en: "Neural Network", def: "模仿大腦神經元連結的數學結構，是深度學習模型的骨架。", see: ["深度學習", "參數"] },
    { term: "多模態", en: "Multimodal", def: "模型能同時處理多種型態的資料——文字、圖片、聲音、影片。現在的 Claude、ChatGPT 都是多模態。", see: ["模型", "LLM 大型語言模型"] },
    { term: "訓練與推論", en: "Training / Inference", def: "「訓練」是用海量資料把模型教出來（很貴、很久）；「推論」是你實際使用它回答問題——你每問一次都算一次推論。", see: ["模型", "Token"] },
    { term: "Git", en: "Git", def: "管理程式碼版本的工具，能記錄每次修改、隨時回到舊版本，是寫程式的基本配備。", see: ["GitHub／儲存庫", "終端機"] },
    { term: "GitHub／儲存庫", en: "GitHub / Repository", def: "GitHub 是放程式碼的雲端平台；一個專案的程式碼集合就叫一個 repo（儲存庫）。本站的程式就放在 GitHub 上。", see: ["Git", "部署"] },
    { term: "套件／程式庫", en: "Package / Library", def: "別人寫好、可以直接拿來用的程式積木。用 npm 或 pip 一行指令就能安裝，不用自己重造輪子。", see: ["npm"] },
    { term: "JSON", en: "JSON", def: "一種電腦之間傳資料的通用格式，長得像有層次的清單。API 回傳的資料大多是 JSON。", see: ["API"] },
    { term: "localhost", en: "localhost", def: "指「你自己這台電腦」。程式在本機跑起來時，通常從 localhost 加一個埠號（如 localhost:3000）打開來看。", see: ["前端 / 後端", "終端機"] },
